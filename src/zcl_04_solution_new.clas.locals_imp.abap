...
CLASS lcl_passenger_flight IMPLEMENTATION.
  ...
  METHOD class_constructor.
    ...
    FIELDS carrier_id, connection_id,
      ...
      CONVERT DATE lv_today
              time connection-departure_time
          time zone lt_airports[ airport_id = connection-airport_from_id ]-timzone
          into UTCLONG DATA(departure_utc).

          CONVERT DATE lv_today
                  TIME connection-arrival_time
                  TIME ZONE lt_airports[ airport_id = connection-airport_to_id ]-timzone
                  INTO UTCLONG data(arrival_utc).
           connection-duration = utclong_diff( high = arrival_utc
                                                low = departure_utc ) / 60.
...
*        INTO @connection_details .
  ...
  METHOD get_description.
    ...
    APPEND |Duration:       { Connection_details-duration / 60 }:{ Connection_details-duration mod 60 }| to r_result.
  ENDMETHOD.
